...
export function TextField(props: ITextFieldProps) {
  const { error, className, isFull, onChange, type, value, list, placeholder, name } = props;
  return (
    <div className={classNames({ "max-w-full w-full": isFull, "w-fit": !isFull })}>
      ...
      />
      {error && <span className="text-sm text-[#ff6166] leading-5 block text-start">{error}</span>}
    </div>
  );
}
